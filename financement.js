...
function simulerFinancement(params){
  ...
  const tarif   = offre === 'acceleree' ? tarifAccelere(kwc, st) : tarifClassique(kwc, st);
  const revenuAn   = prod * tarif;
  const revenuMois = revenuAn / 12;

  const mensualite = mensualitePret(prix, taux, annees);
...
function finRenderModule(hostId, params, options){
  ...
  FIN_STATE[hostId] = Object.assign({
    kwc: 20, prix: 90000, production: 35000,
...
function finPaint(hostId){
  ...
  <div class="fin-wrap">
    ...
    <div class="fin-head">
      <div>
        <div class="fin-title">${finNum(s.kwc)} kWc — ${finShekel(s.prix)}</div>
        ${sousTitre ? `<div class="fin-sub">${sousTitre}</div>` : ''}
      </div>
      <div class="fin-bank">
        <span class="fin-bank-name">Bank Hapoalim</span>
        <span class="fin-bank-rate">${s.taux.toString().replace('.',',')} %</span>
        <span class="fin-bank-formula">Prime + ${st.marge_prime.toString().replace('.',',')} %</span>
      </div>
    </div>
    ...
    <div class="fin-cards">
      <div class="fin-card">
        ...
        <div class="fin-card-note">${finNum(Math.min(s.kwc,st.seuil_kw))} kW à ${(s.offre==='acceleree'?st.tarif_acc_bas:st.tarif_cl_bas).toString().replace('.',',')} ₪${s.kwc>st.seuil_kw?` · ${finNum(s.kwc-st.seuil_kw)} kW à ${st.tarif_haut.toString().replace('.',',')} ₪`:''}</div>
      </div>
      <div class="fin-card">
